...
contract Escrow_v1_0 {
    ...
    *@param moderators List of moderators for this transaction. For now only single moderator
    ...
    */
        ...
        private
    {
        ...
        for (uint256 i = 0;i<moderators.length; i++) {
            ...
            require(moderators[i] != address(0), "Zero address passed");

            //Check if same moderator is not passed twice in the list
            require(
                !transactions[scriptHash].isOwner[moderators[i]], 
                "Moderator is beign repeated"
            );

            transactions[scriptHash].isOwner[moderators[i]] = true;
        }
